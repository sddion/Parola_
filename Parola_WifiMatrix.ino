...
textEffect_t effects[] = {PA_NO_EFFECT,
                          PA_PRINT,
                          PA_SCROLL_UP,
                          PA_SCROLL_DOWN,
                          PA_SCROLL_LEFT,
...
const char *effect_names[] = {"No Effect",
                              "Print",
                              "Scroll Up",
                              "Scroll Down",
                              "Scroll Left",
...
InfoMode infoMode = MODE_TIME;${workspaceRoot}/node_modules/.bin/clang-format
...
const char index_html[] PROGMEM =
    ...
    "<span class='info-title-modern'> State </span>"
    ...
    "};"
    "document.getElementById('reset-button').onclick=function(e){e."
    "preventDefault();"
    "if(!checkAuth()) return;"
    ...
    "Live updated!');});"
    " setFxGallery();"
    " if(checkAuth()){fetchStatus();setInterval(fetchStatus,1000);}"
...
void updateClock() {
  if (timeClient.update()) {
    ...
    char uiBuf[32];
    snprintf(uiBuf, sizeof(uiBuf), "%s, %s %s", dayStr.c_str(), dateBuf,
             timeBuf);
...
void forceShowDefaultMessage() {
  ...
  if (msg.length() == 0) {
    ...
    if (infoMode == MODE_TIME) {
      displayText = lastTimeStr.c_str();
      waitingForEffectFinish = true;
    } else if (infoMode == MODE_DATE) {
      displayText = lastDateStr.c_str();
      waitingForEffectFinish = true;
    } else {
      displayText = lastDayStr.c_str();
      waitingForEffectFinish = true;
    }
    myDisplay.displayText(displayText, PA_CENTER, myDisplay.getSpeed(), 1000,
                          eff, eff);
  } else {
    waitingForEffectFinish = false;
    myDisplay.displayText(settings.message, PA_LEFT, myDisplay.getSpeed(), 0,
                          eff, eff);
  }
}
...
void setup() {
  ...
  server.on("/status", HTTP_GET, []() {
    ...
    json += "\"message\":\"" + String(sanitizeMsg(String(settings.message))) +
            "\",";
    ...
    char uptime_str[24];
    snprintf(uptime_str, sizeof(uptime_str), "%02luh %02lum %02lus", hrs, min,
             s);
    json += "\"uptime\":\"";
    json += uptime_str;
    json += "\"";
